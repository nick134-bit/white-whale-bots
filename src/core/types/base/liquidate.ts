	...
	for (const tx of mempool.txs) {
		...
		for (const message of txRaw.body.messages) {
			...
			if (liquidate.feeder[msgExecuteContract.sender]) {
				...
				liquidate.prices[msgExecuteContract.contract] ||
				liquidate!.moneymarkets[msgExecuteContract.contract]
			) {
				liqChanges[1].push({
	...
	if (mempooltxs[1].length > 0) {
		for (let i = 0; i < mempooltxs[1].length; i++) {
			if (mempooltxs[1][i].message.lock_collateral) {
				console.log(JSON.stringify(mempooltxs[1][i]));
				console.log(mempooltxs[1][i].message.lock_collateral);
				mempooltxs[1][i].message.lock_collateral.collaterals.forEach(async (elem: [string, string]) => {
					console.log(loansObj[mempooltxs[1][i].overseer][mempooltxs[1][i].address]);
					console.log(loansObj[mempooltxs[1][i].overseer][mempooltxs[1][i].address].collaterals);
				...
				console.log(JSON.stringify(mempooltxs[1][i]));
				console.log(mempooltxs[1][i].message.unlock_collateral);
				mempooltxs[1][i].message.unlock_collateral.collaterals.forEach(async (elem: [string, string]) => {
					console.log(loansObj[mempooltxs[1][i].overseer][mempooltxs[1][i].address]);
					console.log(loansObj[mempooltxs[1][i].overseer][mempooltxs[1][i].address].collaterals);
					if (loansObj[mempooltxs[1][i].overseer][mempooltxs[1][i].address]) {
						if (loansObj[mempooltxs[1][i].overseer][mempooltxs[1][i].address].collaterals![elem[0]]) {
...
function addCollateral(
	...
	if (loansObj[overseer][address].collaterals) {
		if (loansObj[overseer][address].collaterals![toAdd[0]]) {
			//console.log("add to: " + JSON.stringify(loansObj[overseer][address].collaterals!));
			//console.log("added: " + toAdd + " to: " + address);
			loansObj[overseer][address].collaterals![toAdd[0]] =
				loansObj[overseer][address].collaterals![toAdd[0]]! + Number(toAdd[1]);

			//console.log("added to: " + JSON.stringify(loansObj[overseer][address].collaterals!));
		} else {
			loansObj[overseer][address].collaterals![toAdd[0]] = Number(toAdd[1]);
		}
	}
